...
// SoftmaxPair loss
template <typename Dtype> class SoftmaxPairLayer;

template <typename Dtype>
class SoftmaxPairWithLossLayer : public LossLayer<Dtype> {
 public:
  explicit SoftmaxPairWithLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "SoftmaxPairWithLoss"; }
  virtual inline int MaxTopBlobs() const { return 2; }
 ...
 protected:
  ...
  // The internal SoftmaxPairLayer used to map predictions to a distribution.
  shared_ptr<Layer<Dtype> > softmax_pair_layer_;
  // prob stores the output probability of the layer.
  Blob<Dtype> prob_;
  // Vector holders to call the underlying softmax layer forward and backward.
  vector<Blob<Dtype>*> softmax_pair_bottom_vec_;
  vector<Blob<Dtype>*> softmax_pair_top_vec_;
  int softmax_pair_axis_;
};
