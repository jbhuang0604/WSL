...
// Get softmax pair layer according to engine.
template <typename Dtype>
shared_ptr<Layer<Dtype> > GetSoftmaxPairLayer(const LayerParameter& param) {
  SoftmaxPairParameter_Engine engine = param.softmax_pair_param().engine();
  if (engine == SoftmaxPairParameter_Engine_DEFAULT) {
    engine = SoftmaxPairParameter_Engine_CAFFE;
  }
  if (engine == SoftmaxPairParameter_Engine_CAFFE) {
    return shared_ptr<Layer<Dtype> >(new SoftmaxPairLayer<Dtype>(param));
  } else {
    LOG(FATAL) << "Layer " << param.name() << " has unknown engine.";
  }
}

REGISTER_LAYER_CREATOR(SoftmaxPair, GetSoftmaxPairLayer);
